...
if ${_INSTALL_SOLR} = true; then
    ...
    # SolR 5.5.1 Default home is /var/solr
    ...
    if ! sudo bash ./install_solr_service.sh /tmp/solr-$_SOLR_VERSION.tgz -p $_SOLR_PORT >/dev/null 2>&1; then
    ...
    if ! sudo su - solr -c "/opt/solr/bin/solr create_core -c $_SOLR_CORE" < /dev/null 2>&1; then
